...
module private Process =
    ...
    let drawTimeDigits offsetY (hourOffsetX, image) =
        ...
        let draw image (hour, offsetX) =
            drawText (hourStr hour) Color.Cyan (offsetX, offsetY) image

        (hourOffsetX, hourOffsetX |> List.filter thirdHour |> List.fold draw image)

    let drawTimeTicks offsetY label (hourOffsetX, image) =
        let pen hour =
            if hour % 3.0<h> = 0.0<h> then penSolidTick else penDotTick
    ...
    let last24hourOffsets () =
        ...
        let toHourOffset hour =
            (hour, (hour - hourNow) * pixelsPerHour - minuteOffset)

        let inDrawArea (_, offsetX) =
            offsetX >= 0.0<px> && offsetX <= 336.0<px>
    ...
    let timeOffset timestamp =
        344.0<px>
        + ((timestamp - DateTime.Now).TotalMinutes * 1.0<minute> * pixelsPerMinute)
    ...
    let drawWindSpeedGraph data image =
        ...
        List.fold plotMax image data |> List.fold plotAverage <| data
    ...
    let drawPressureGraph data image =
        let (minPressure, maxPressure) =
            data |> maxSpanBy (fun x -> x.AirPressureSeaLevel) 5.0<hPa> 20.0<hPa>
        ...
        let plot image value =
            ...
            | Some pressure ->
                plot penNormalGraph value.Timestamp (108.0<px> - (pressure - minPressure) * pixelsPerhPa) image
            | None -> image
    ...
    let drawTemperatureGraph data image =
        let (minTemp, maxTemp) =
            data |> maxSpanBy (fun x -> x.TemperatureAir) 5.0<degC> 20.0<degC>
    ...
    let drawMeter<[<Measure>] 'u>
        (projection: Input.SensorData -> float<'u> option)
        (toAngle: float<'u> -> float)
        draw
        data
        image
        =
    ...
    let drawAirPressureMeter data image =
        ...
        let pressureToAngle (pressure: float<hPa>) =
            ((pressure - 1000.0<hPa>) * 240.0 / 80.0<hPa>) * Math.PI / 180.0
    ...
    let drawTemperatureMeter data image =
        ...
        let temperatureToAngle (temperature: float<degC>) =
            (temperature * 240.0 / 60.0<degC>) * Math.PI / 180.0
