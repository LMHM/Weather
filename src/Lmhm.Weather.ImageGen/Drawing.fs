...
let font =
    use fontStream =
        Assembly
            ...
            .GetManifestResourceStream("Lmhm.Weather.ImageGen.consola.ttf")
...
let loadImage resourceName =
    use imageStream =
        Assembly
            .GetExecutingAssembly()
            .GetManifestResourceStream("Lmhm.Weather.ImageGen." + resourceName)
...
let drawText (text: string) (color: Color) (point: Point) (image: Image) =
    image.Mutate(fun ctx ->
        ctx.DrawText(text, font, color, PointF(fst point |> float32, snd point |> float32))
        |> ignore)
...
let drawPoint (pen: Pen) (point: Point) (image: Image) = drawLine pen point point image
